<template>
  <div style="height:100%;width: 100%;position: relative;overflow: hidden">
    ...
    <div id="ColorForm">
      <ColorForm @submit="submit"></ColorForm>
    </div>
...
import Settings from "@/components/elevationgrad/Settings";
import Tile from "ol/layer/Tile";
import {XYZ} from "ol/source";
import {config} from "@/global";
import {Feature, Map, Overlay, View} from "ol";
import ColorForm from "@/components/elevationgrad/ColorForm";
...
export default {
  name: "Map",
  components: {Settings, ColorForm},
  data() {
    return {
      map: null,
      settingsVisible: false,
      urlTemplate: config.elevationGradUrl + "/tile.webp?z={z}&x={x}&y={y}",
    }
  },
  watch: {},
  computed: {},
  methods: {
    ...
    init() {
      ...
      let overlayLayer = new VectorLayer({source, name: "overlay", zIndex: 100});
      this.map.addLayer(overlayLayer);
      this.map.on('singleclick', e => {
        let position = [e.coordinate[0], e.coordinate[1]];
        axios.get(config.elevationGradUrl + "/rasterValue.json?x=" + position[0] + "&y=" + position[1]).then(result => {
    ...
    initLayer(urlTemplate = this.urlTemplate) {
      return new Tile({
        source: new XYZ({
          url: urlTemplate,
          projection: "EPSG:4326",
    ...
    submit(colorTable) {
      ...
      let urlTemplate = config.elevationGradUrl + "/tile.png?z={z}&x={x}&y={y}&renderingRule=" + JSON.stringify(renderingRule);
      console.log(urlTemplate);
      this.map.addLayer(this.initLayer(urlTemplate));
    },
